...
def export_to_xls_or_csv(df: pl.DataFrame, fname: str):
    ...
    try:
        df_pd.to_excel(
            osp.join(gdrive_path, f"{fname}.xlsx"), index=False, float_format="%.4f"
        )
    except ValueError:
        print("file too large, use csv")
        df_pd.to_csv(
            osp.join(gdrive_path, f"{fname}.csv"), index=False, float_format="%.4f"
        )
...
global_shape = (
    data.filter(pl.col("year").is_in(range(2021, 2101)))
    .group_by(["year", "bracket"])
    .agg(pl.col("population").sum())
    .with_columns(pl.lit("world").alias("global"))
    .sort(("year", "bracket"))
)
global_shape
...
# country shape 2023
country_shape = data.filter(pl.col("year") == 2023).sort(("year", "bracket"))
...
def resample_country(df, rmax=None, scale=10):  # will it work if scale = 20?
    ...
    year = df["year"][0]
    df_ = (
        df.with_columns((pl.col("bracket") / scale).cast(pl.Int32).alias("bnew"))
        .group_by(["bnew"], maintain_order=True)
        .agg(pl.col("population").sum())
        .select(pl.col("bnew").alias("bracket"), pl.col("population"))
    )
    if rmax is None:
        full_range = pl.Series(
            "bracket", range(df_["bracket"].max()), dtype=pl.Int32
        ).to_frame()
    else:
        full_range = pl.Series("bracket", range(rmax), dtype=pl.Int32).to_frame()

    return (
        df_.join(full_range, on="bracket", how="outer")
        .with_columns(
...
# world in 105
world105 = (
    country105.group_by(["year", "bracket"])
    .agg(pl.col("population").sum())
    .select(pl.lit("world").alias("global"), pl.all())
    .sort(["year", "bracket"])
)
world105
...
# country in 50
country50 = country105.filter(pl.col("bracket") < 50)
country50
export_to_xls_or_csv(country50, "country_brackets_50_pop")

# world in 50
world50 = (
    country50.group_by(["year", "bracket"])
    .agg(pl.col("population").sum())
    .select(pl.lit("world").alias("global"), pl.all())
    .sort(["year", "bracket"])
)
...
def get_richest_income(
    ser,  # input: population count by bracket
    splitnum: int,
    kw: str,
    cut=False,
):
...
def get_richest_1(
    ser, country, year, cut=False
):  # divide income mountain into 1000 parts and get richest
    return get_richest_income(
        ser.select(["bracket", "population"]), 1000, "t_0_001", cut
    ).with_columns(pl.lit(country).alias("country"), pl.lit(year).alias("year"))


def get_richest_2(
    ser, country, year, cut=False
):  # divide income mountain into 100000 parts and get richest
    return get_richest_income(
...
def get_split_income(
    ...
    idxs = map(lambda x: tmpl.format(kw[0], x + 1), range(splitnum))
    res_df = pl.DataFrame(dict([(kw, idxs), ("income", res)]))
    return res_df
...
export_to_xls_or_csv(
    decile_income.select(["country", "year", "decile", "income"]), "decile_hhinc"
)
export_to_xls_or_csv(
    ventile_income.select(["country", "year", "ventile", "income"]), "ventile_hhinc"
)
export_to_xls_or_csv(
    quintile_income.select(["country", "year", "quintile", "income"]), "quintile_hhinc"
)
export_to_xls_or_csv(
    centile_income_22.select(["country", "year", "centile", "income"]),
    "centile_hhinc_22",
)

# centile for 1940-2023
data_part_4023 = data.filter(pl.col("year").is_in(range(1940, 2024))).partition_by(
    ["country", "year"], as_dict=True
)

centile_income_4023 = extract_income_rank(get_centile_income, data_part_4023)
# centile_income_4023

res4023 = centile_income_4023.pivot(
    values="income", index=["country", "centile"], columns="year"
)
res4023
export_to_xls_or_csv(res4023, "centile_mean_income_4023")

# centile for 2024-2100
data_part_2400 = data.filter(pl.col("year").is_in(range(2024, 2101))).partition_by(
    ["country", "year"], as_dict=True
)
...
res2400 = centile_income_2400.pivot(
    values="income", index=["country", "centile"], columns="year"
)
...
gdppc = gdppc.select(
    pl.col("geo").alias("country"),
    pl.col("time").alias("year"),
    (pl.col("Income per person") / 365.0).alias("gdppc"),
)
gdppc

mean_income = meaninc.join(
    gdppc, on=["country", "year"], how="outer"
...
quintile_gdp = (
    quintile_income.join(ratio, on=["country", "year"], how="inner")
    .with_columns(pl.col("income") * pl.col("ratio"))
    .select(pl.exclude("ratio"))
)
ventile_gdp = (
    ventile_income.join(ratio, on=["country", "year"], how="inner")
    .with_columns(pl.col("income") * pl.col("ratio"))
    .select(pl.exclude("ratio"))
)
decile_gdp = (
    decile_income.join(ratio, on=["country", "year"], how="inner")
    .with_columns(pl.col("income") * pl.col("ratio"))
    .select(pl.exclude("ratio"))
)
centile_gdp_22 = (
    centile_income_22.join(ratio, on=["country", "year"], how="inner")
    .with_columns(pl.col("income") * pl.col("ratio"))
    .select(pl.exclude("ratio"))
)

rich_gdp = (
    rich_hhinc.join(ratio, on=["country", "year"], how="inner")
    .with_columns(pl.col("income") * pl.col("ratio"))
    .select(pl.exclude("ratio"))
)
...
export_to_xls_or_csv(rich_gdp, "richest_gdp")
...
def get_median_income(df):
    dfc = df.with_columns((pl.col("population").cumsum()).alias("cumsum"))
    b0 = dfc.filter(pl.col("cumsum") <= 0.5)["bracket"][-1]

    b1 = dfc.filter(pl.col("cumsum") >= 0.5)["bracket"][0]
...
for k, v in data_pct.partition_by(["country", "year"], as_dict=True).items():
    ...
    median_income.append(
        v.select(pl.col(["country", "year"]).first(), pl.lit(m).alias("median_income"))
    )
median_income = pl.concat(median_income)
median_income

# gdppc version
median_gdppc = median_income.join(ratio, on=["country", "year"], how="inner").select(
    pl.col(["country", "year"]),
...
epov_rate = pl.read_csv(
    "../../poverty_rates/ddf--datapoints--poverty_rate--by--country--time.csv"
)
export_to_xls_or_csv(epov_rate, "extreme_poverty_rate")

epov_pop = pl.read_csv(
    "../../poverty_rates/ddf--datapoints--population_in_extreme_poverty--by--country--time.csv"
)
export_to_xls_or_csv(epov_pop, "people_in_extreme_poverty")

epov_rate_gbl = pl.read_csv(
    "../../poverty_rates/ddf--datapoints--poverty_rate--by--global--time.csv"
)
export_to_xls_or_csv(epov_rate_gbl, "extreme_poverty_rate_global")

epov_rate_level = pl.read_csv(
    "../../poverty_rates/ddf--datapoints--poverty_rate--by--income_groups--time.csv"
)
