...
gbl_total_pop = total_pop.group_by("year").agg(pl.col("population").sum())
gbl_total_pop
...
# where is 3/day
upper_bracket = bracketlib.bracket_from_income(3, step)
...
def get_epov_rates(df, xloc, lb, ub, rates=True):
    ...
    df_ = (
        ...
        .filter(pl.col("bracket").is_in([lb, ub]))
        .with_columns(
            # note: the log base should be the same as how xloc calculated
            pl.col("bracket").map_elements(income_from_bracket, return_dtype=pl.Float64)
        )
    )
    if not rates:
        df_ = df_.with_columns((pl.col("poverty_pop") * total_pop_data).floor().cast(pl.Int64))
    if df_.is_empty():
        if rates:
            return 0.0
        return 0
...
get_epov_rates(
    data_gbl.filter(pl.col("year") == 2021),
    lb=lower_bracket,
    ub=upper_bracket,
    xloc=xloc,
    rates=True,
...
def get_epov_rates_for_groups(df, by, povline, rates=True):
    ...
    for k, _df in datalist.items():
        ...
        res_part[name] = get_epov_rates(_df, lb=lb, ub=ub, xloc=xloc, rates=rates)
        res.append(res_part)

    return pl.from_records(res)


gbl_epov_rates = get_epov_rates_for_groups(data_gbl, ["year"], 3).sort("year")
...
).join(gbl_epov_rates_685, on=["global", "time"], how="full", coalesce=True)
global_rates
...
country_epov_rates = get_epov_rates_for_groups(data, ["country", "year"], 3)
country_epov_rates = country_epov_rates.rename({"year": "time"})
country_epov_rates

country_epov_rates.filter((pl.col("country") == "sle") & (pl.col("time") == 1981))
# => 64.22, correct!

country_epov_rates.filter((pl.col("country") == "nru") & (pl.col("time") == 2009))

country_epov_rates.write_csv(
    "./ddf/poverty_rates/ddf--datapoints--extereme_poverty_rate--by--country--time.csv"
)

# I also need the population in extreme poverty
country_epov_pop = get_epov_rates_for_groups(data, ["country", "year"], 3, rates=False)
country_epov_pop = country_epov_pop.rename({"year": "time"})

country_epov_pop.write_csv(
...
).join(country_epov_rates_685, on=["country", "time"], how="full", coalesce=True)
country_rates
...
on_map = {
    ...
    "Level 3": "l3",
    "Level 4": "l4",
    "Level 5": "l4",  # map level 5 and above to high income
    "Level 6": "l4",
    "Level 7": "l4",
}
...
income_3groups = income_groups.select(
    ...
    pl.col("level").replace(
        {"upper_middle_income": "middle_income", "lower_middle_income": "middle_income"}
    ),
)
income_3groups
...
data_income_3level = (
    data.join(income_3groups, on=["country", "year"], how="left")
    .group_by(pl.col(["level", "year", "bracket"]))
    .agg(pl.col("population").sum())
    .sort(["level", "year", "bracket"])
)
...
def get_level_epov_rates(povline, use_3levels=False):
    ...
    return rates.select(
        pl.col("level").alias("income_group"), pl.col("year").alias("time"), pl.col(name)
    )


level_epov_215 = get_level_epov_rates(3)
level_epov_215

level_epov_215.write_csv(
    "./ddf/poverty_rates/ddf--datapoints--poverty_rate--by--income_groups--time.csv"
)

level_epov_215_3lvl = get_level_epov_rates(3, use_3levels=True)
level_epov_215_3lvl

level_epov_215_3lvl.write_csv(
    "./ddf/poverty_rates/ddf--datapoints--poverty_rate--by--income_3groups--time.csv"
)
...
).join(level_epov_685, on=["income_group", "time"], how="full", coalesce=True)
level_rates
...
).join(level_epov_685_3lvl, on=["income_group", "time"], how="full", coalesce=True)
level_rates_3lvl
...
# 2.15
region_epov = get_epov_rates_for_groups(data_world_4region, ["world_4region", "year"], 3)
region_epov
...
).join(region_epov_685, on=["world_4region", "year"], how="full", coalesce=True)
region_rates
...
pop_in_epov_2 = epov_under_2.join(population, on=["country", "year"], how="left").select(
    pl.col(["country", "year"]),
    (pl.col("population") * pl.col("poverty_rate_under_2") / 100)
    .cast(pl.Int64)
    .alias("population_under_2"),
)
pop_in_epov_2
...
pop_above_200 = epov_under_200.join(population, on=["country", "year"], how="left").select(
    pl.col(["country", "year"]),
    (pl.col("population") * (1 - pl.col("poverty_rate_under_200") / 100))
    .cast(pl.Int64)
    .alias("population_above_200"),
)
pop_above_200
...
pop_global = (
    pop.group_by("year")
    .agg(pl.col("population_under_2").sum(), pl.col("population_above_200").sum())
    .select(pl.lit("world").alias("global"), pl.col("*"))
    .sort("year")
)
pop_global
