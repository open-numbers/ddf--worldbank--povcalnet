...
# FIXME: change all dimensions to geo,time
...
def format_numeric_column(df, column_name, decimals=3):
    """Round a numeric column to specified number of decimals."""
    return df.with_columns(pl.col(column_name).round(decimals))
...
def get_decile_income(ser, country, year, cut=False):
    return get_split_income(ser.select(["bracket", "population"]), 10, "decile", cut).with_columns(
        pl.lit(country).alias("country"), pl.lit(year).alias("year")
    )


def get_ventile_income(ser, country, year, cut=False):
    return get_split_income(ser.select(["bracket", "population"]), 20, "ventile", cut).with_columns(
        pl.lit(country).alias("country"), pl.lit(year).alias("year")
    )
...
def get_quintile_income(ser, country, year, cut=False):
    return get_split_income(ser.select(["bracket", "population"]), 5, "quintile", cut).with_columns(
        pl.lit(country).alias("country"), pl.lit(year).alias("year")
    )
...
# export to file
# os.makedirs('other/')
decile_income.select(["country", "year", "decile", "income"]).write_csv(
    "other/decile_mean_income.csv"
)
ventile_income.select(["country", "year", "ventile", "income"]).write_csv(
    "other/ventile_mean_income.csv"
)
quintile_income.select(["country", "year", "quintile", "income"]).write_csv(
    "other/quintile_mean_income.csv"
)
centile_income_22.select(["country", "year", "centile", "income"]).write_csv(
    "other/centile_mean_income_22.csv"
)
...
res4023 = centile_income_4023.pivot(values="income", index=["country", "centile"], on="year")
...
res2400 = centile_income_2400.pivot(values="income", index=["country", "centile"], on="year")
...
mean_income = meaninc.join(gdppc, on=["country", "year"], how="full")  # there are some nulls
mean_income = mean_income.drop_nulls()
mean_income

mean_income = mean_income.with_columns((pl.col("gdppc") / pl.col("mhhinc")).alias("ratio"))

ratio = mean_income.select(["country", "year", "ratio"]).with_columns(pl.col("year").cast(pl.Int32))
...
centile_gdp_22.select(["country", "year", "centile", "income"]).write_csv(
    "other/centile_gdp_22.csv"
)
...
def get_median_income(df):
    dfc = df.with_columns((pl.col("population").cum_sum()).alias("cumsum"))
...
median_gdppc = median_income.join(ratio, on=["country", "year"], how="inner").select(
    pl.col(["country", "year"]), (pl.col("median_income") * pl.col("ratio")).alias("median_gdppc")
)
median_gdppc
...
# export to file
format_numeric_column(median_income, "median_income").write_csv("other/median_income.csv")
format_numeric_column(median_gdppc, "median_gdppc").write_csv("other/median_gdp.csv")
