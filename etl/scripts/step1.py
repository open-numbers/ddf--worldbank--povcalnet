...
4. convert geo codes into open_numbers geo codes.
"""
...
def load_file_preprocess(filename):
    ...
    df = df.rename({"country_code": "country", "reporting_year": "year"})
    # check if there are duplicates
    dups = df.filter(df.select(["country", "reporting_level", "year"]).is_duplicated())
...
def step2(res1):
    ...
    for i, df in res1.items():
        ...
        df_new = df_new.select(
            [
                pl.col(["country", "year", "reporting_level", "i", "reporting_pop", "headcount"]),
            ]
        )
        res.append(df_new)
...
def run_fill_df(df_input: pl.DataFrame, fillna_til):
    ...
    if df.select("headcount").null_count().item() > 0:
        ...
        return df.with_columns([pl.col("headcount").map_batches(lambda x: run_fill(x, til))])
    else:
        return df
...
def step3(res2):
    ...
    _missing = (
        res2.filter(pl.col("headcount").is_null())
        .select(pl.col(["country", "year", "reporting_level"]))
        .unique()
    )
    ...
    _missing_years = _missing.group_by("country", "reporting_level").agg(
        [pl.col("year").min().alias("min_year"), pl.col("year").max().alias("max_year")]
    )
...
# %%
def step4(res3):
    # Read synonyms file and create mapping dictionary
    synonyms_file = "source/open_numbers/ddf--synonyms--geo.csv"
    synonyms_df = pl.read_csv(synonyms_file)

    # Create dictionary with synonym as key and geo as value
    geo_mapping = dict(zip(synonyms_df["synonym"], synonyms_df["geo"]))

    # Get unique country codes from res3
    unique_countries = res3.select("country").unique().to_series().to_list()

    # Check for missing mappings and raise error if any found
    missing_countries = [country for country in unique_countries if country not in geo_mapping]

    if missing_countries:
        print(
            f"Error: {len(missing_countries)} country code(s) in data not found in synonyms dictionary:"
        )
        print(f"Missing countries: {sorted(missing_countries)}")
        raise KeyError(
            f"Country codes not found in synonyms dictionary: {sorted(missing_countries)}"
        )

    # Apply the mapping to the country column (strict mode - will raise if key not found)
    res4 = res3.with_columns([pl.col("country").replace_strict(geo_mapping).alias("country")])
...
if __name__ == "__main__":
    ...
    res3 = step3(res2)
    # 4. convert ISO3 code to open_numbers geo code
    res4 = step4(res3)
    assert res4.shape == res2.shape
    assert res4.filter(pl.col("headcount") > 1).is_empty()
    assert res4.filter(pl.col("headcount") < 0).is_empty()
    # save result to parquet
    res4.write_parquet("./povcalnet_clean.parquet")
