...
def move_mean(df: pl.DataFrame):
    ...
    if inc:
        ...
        return df.with_columns([pl.col("bracket") - brack])
    else:
        print(c, y)
        # no available income data, just return an empty dataframe.
        return df.select([pl.exclude("headcount")]).with_columns(pl.lit(np.nan).alias("headcount"))
...
if __name__ == "__main__":
    ...
    res7 = res7.with_columns(pl.col("year").cast(pl.Int32))
    res8 = step8(res7)
