...
def calculate_bridge(left, right, scale=1500):
    ...
    bridge_start = (
        left.filter((pl.col("population") > thres)).select(pl.col("bracket").last()).item()
    )
    left1 = left.filter(pl.col("bracket").is_between(left_top_x, bridge_start)).select(
        ["bracket", "population"]
    )
    ...
    right1 = pl.DataFrame({"bracket": right_start, "population": right_max}, schema=left1.schema)

    middle = right_start + (right_span // 2)
    right_idx = pl.Series("bracket", range(right_start, right_end + 1), dtype=pl.Int32).to_frame()
    bridge_end_y = (
        right.join(right_idx, on="bracket", how="full", coalesce=True)
        .with_columns(pl.col("population").interpolate())
        .filter(pl.col("bracket").is_in(range(middle - 5, middle + 5 + 1)))["population"]
        .mean()
    )

    right2 = pl.DataFrame({"bracket": middle, "population": int(bridge_end_y)}, schema=left1.schema)

    new_i = pl.Series("bracket", range(left_top_x, middle + 1), dtype=pl.Int32).to_frame()
    res = pl.concat([left1, right1, right2]).with_columns(np.log(pl.col("population")))
    res = res.join(new_i, on="bracket", how="full", coalesce=True)
    res = res.with_columns(np.exp(pl.col("population").map_batches(interpolate)))
    ...
    res = res.with_columns(pl.col("population").round(0).cast(pl.Int64))

    bridge_shape_all = (
        left.select(["bracket", "population"])
        .join(
        ...
        .join(res, on="bracket", how="full", suffix="_bridge", coalesce=True)
    )
    bridge_shape_all = bridge_shape_all.select(
        pl.col("bracket"),
        pl.coalesce(
            pl.col(["population_bridge", "population_right", "population"]), pl.lit(0)
        ).alias("population"),
    ).sort("bracket")
...
def calculate_bridge_2(left_shape, params):
    ...
    try:
        bridge_start_x = left_shape_.filter(
            pl.col("population") <= thres, pl.col("population") > 0
        )["bracket"][0]  # should I check indexerror here?
    except IndexError:
        return None

    bridge_first_val = np.log(
        left_shape.filter(pl.col("bracket") == bridge_start_x)["population"].item()
    )
    bridge_vals = list()
    slope = params["slope"]
    ...
    brackets = np.arange(bridge_start_x, len(bridge_arr) + bridge_start_x).astype(np.int32)
    bridge_shape = pl.DataFrame({"bracket": brackets, "population": bridge_arr})
    left_shape1 = left_shape.filter(pl.col("bracket") < bridge_start_x).select(
        ["bracket", "population"]
    )
    bridge_shape_all = pl.concat([left_shape1, bridge_shape])
    return fix_total_pop(left_shape, bridge_shape_all.sort("bracket"))
...
def fix_total_pop(left, res):
    ...
    res_ = res.join(left_diff, on="bracket", how="left").select(
        pl.col("bracket"),
        (pl.col("population") - pl.col("population_right").fill_null(0)).alias("population"),
    )
...
def make_checking_plots(povcalnet, billy_pop, all_shapes):
    ...
    gleft = povcalnet.group_by(["year", "bracket"]).agg(pl.col("population").sum())
    gbridge = all_shapes.group_by(["year", "bracket"]).agg(pl.col("population").sum())
    gright = billy_pop.group_by(["year", "bracket"]).agg(pl.col("population").sum())
    ...
    for t in ts:
        ...
        diff = (
            (bridge["population"].sum() - left["population"].sum()) / left["population"].sum() * 100
        )
        ...
        plot_shape(right, alpha=0.4, label="billy")
        ax.set_yscale("log")
        # ax.set_ylim((0, 1000))
...
if __name__ == "__main__":
    ...
    billy = pl.read_csv("./source/billionaire/ddf--datapoints--daily_income--by--person--time.csv")
    billy_worth = pl.read_csv("./source/billionaire/ddf--datapoints--worth--by--person--time.csv")
    billy = billy.with_columns(
        pl.col("time").cast(pl.Int32), pl.col("daily_income").cast(pl.Float64)
    )
    billy_worth = billy_worth.with_columns(
        pl.col("time").cast(pl.Int32), pl.col("worth").cast(pl.Float64)
    )
    billy_ent = pl.read_csv(
        "./source/billionaire/ddf--entities--person.csv", infer_schema_length=None
    )
    billy_ent = billy_ent.select(
        ["person", "country"]
    ).drop_nulls()  ## remove person with no country info.

    billy_country_map = dict([(d["person"], d["country"]) for d in billy_ent.to_dicts()])
    ...
    df_ann = df_ann.with_columns((np.log(pl.col("2022") / pl.col("2012")) / 10).alias("growth"))
    df_ann
    ...
    billy_pop = (
        billy_full.with_columns(
            pl.col("person").replace_strict(billy_country_map, default=None).alias("country"),
            (
                pl.col("daily_income")
        ...
        .drop_nulls(subset=["country"])
        .group_by(["country", "time", "bracket"])
        .agg(pl.col("daily_income").count().alias("population"))
        .sort(["country", "time", "bracket"])
    )
    billy_pop.columns = ["country", "year", "bracket", "population"]

    billy_pop = billy_pop.with_columns(pl.col("population").cast(pl.Int64))
    ...
    print(
        "please double check above country/year pairs which the bridging failed. "
    ...
    gini_pl = pl.read_csv("./source/gapminder/gini.csv").select(["geo", "time", "gini_2100"])
    gini_pl = gini_pl.rename({"geo": "country", "time": "year", "gini_2100": "gini"})

    # Convert params_cache to polars DataFrame
    params_cache_list = [
        {
            "country": k[0],
            "year": k[1],
            "slope": v["slope"],
            "bridge_left_y": v["bridge_left_y"],
            "mountain_top_y": v["mountain_top_y"],
        }
        for k, v in params_cache.items()
    ]
    params_cache_pl = pl.DataFrame(params_cache_list)

    # Perform inner join to only keep rows where both params and gini data are available
    params_cache_pl = params_cache_pl.join(gini_pl, on=["country", "year"], how="inner")

    params_cache_pl = params_cache_pl.with_columns(
        (pl.col("bridge_left_y").log() / pl.col("mountain_top_y").log()).alias("ptc_top")
    )

    # Convert to pandas for remaining code
    params_cache_df = params_cache_pl.to_pandas().set_index(["country", "year"])
    gini = params_cache_df["gini"]
    ...
    mid_gini_params = params_cache_df[
        params_cache_df["gini"].between(33, 50)
    ]  # note: inclusive = both
    ...
    all_bridges = (
        pl.concat(
            [
                df.with_columns(
                    pl.lit(k[0]).alias("country"), pl.lit(k[1]).alias("year").cast(pl.Int32)
                )
                for k, df in bridge_cache.items()
            ]
        )
