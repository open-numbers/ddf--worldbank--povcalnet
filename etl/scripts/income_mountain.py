...
def resample_country(df, rmax=None, scale=10):  # will it work if scale = 20?
    ...
    return (
        df_.join(full_range, on="bracket", how="right")
        .with_columns(
...
# global shapes
res_gbl = res.group_by(["year", "bracket"], maintain_order=True).agg(pl.col("population").sum())
res_gbl = res_gbl.select(pl.lit("world").alias("global"), pl.all())
res_gbl
...
res_gbl.select(
    ...
    pl.col("population").alias("income_mountain_105brackets"),
).write_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_105brackets--by--global--time--income_bracket_105.csv"
)
...
res_pivot_gbl = res_gbl.pivot(
    values="population", index=["global", "year"], on="bracket", aggregate_function=None
)
...
out_gbl = res_pivot_gbl.select(
    pl.col("global"),
    pl.col("year").alias("time"),
    pl.struct(pl.col(map(str, (range(0, 105)))))
    .map_elements(join_str, return_dtype=pl.Utf8)
    .alias("income_mountain_105bracket_shape_for_log"),
)
out_gbl

out_gbl.write_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_105bracket_shape_for_log--by--global--time.csv"
)

res_pivot = res.pivot(
    values="population", index=["country", "year"], on="bracket", aggregate_function=None
)
...
out.write_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_105bracket_shape_for_log--by--country--time.csv"
)

# so we will drop those bracket > 49 to create a version of world shape in 50 groups.
res50 = res.filter(pl.col("bracket") < 50)
# res50

res50_pivot = res50.pivot(
    values="population", index=["country", "year"], on="bracket", aggregate_function=None
)
...
out50 = res50_pivot.select(
    pl.col("country"),
    pl.col("year").alias("time"),
    pl.struct(pl.col(map(str, (range(0, 50)))))
    .map_elements(join_str, return_dtype=pl.Utf8)
    .alias("income_mountain_50bracket_shape_for_log"),
)
# out50
out50.write_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_50bracket_shape_for_log--by--country--time.csv"
)
...
ig = [_d.get((row["geo"], row["time"]), None) for _, row in df.iterrows()]

df["income_groups"] = ig

# Drop rows where income group is None and show a warning
missing_rows = df[df["income_groups"].isna()]
if missing_rows.shape[0] > 0:
    print(
        f"Warning: Dropped {missing_rows.shape[0]} rows with missing income group classifications"
    )
    missing_combos = missing_rows[["geo", "time"]].drop_duplicates()
    print(f"Missing income groups for {missing_combos.shape[0]} unique (geo, time) combinations")

df_with_ig = df.dropna(subset=["income_groups"])

res_ig = df_with_ig.groupby(by=["income_groups", "time", "income_bracket_50"])["population"].sum()
...
res_ig.to_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_50bracket_shape_for_log--by--income_groups--time.csv"
)

# income 3 groups

_d = income_3groups["level"].to_dict()
ig3 = [_d.get((row["geo"], row["time"]), None) for _, row in df.iterrows()]

df["income_3groups"] = ig3

# Drop rows where income group is None and show a warning
missing_rows_3g = df[df["income_3groups"].isna()]
if missing_rows_3g.shape[0] > 0:
    print(
        f"Warning: Dropped {missing_rows_3g.shape[0]} rows with missing income 3-group classifications"
    )
    missing_combos_3g = missing_rows_3g[["geo", "time"]].drop_duplicates()
    print(
        f"Missing income 3-groups for {missing_combos_3g.shape[0]} unique (geo, time) combinations"
    )
    print(missing_combos_3g["geo"].unique())

df_with_ig3 = df.dropna(subset=["income_3groups"])

res_ig3 = df_with_ig3.groupby(by=["income_3groups", "time", "income_bracket_50"])[
    "population"
].sum()
...
res_ig3.to_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_50bracket_shape_for_log--by--income_3groups--time.csv"
)
...
res_glob.to_csv(
    "ddf/income_mountain/ddf--datapoints--income_mountain_50bracket_shape_for_log--by--global--time.csv"
)
...
for k in [
    ...
    res_grp.to_csv(
        f"ddf/income_mountain/ddf--datapoints--income_mountain_50bracket_shape_for_log--by--{k}--time.csv"
    )
